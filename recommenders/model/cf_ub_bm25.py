...
class CF_UB_BM25:
        def __init__(self, urm, pop=None, binary=False, verbose=True, mode='offline', datareader=None, verbose_evaluation=True, similarity='tversky'):
                assert(mode in ('offline', 'online'))
                if binary: urm.data=np.ones(urm.data.shape[0])
                if pop is None: self.pop = urm.sum(axis=1).A1 #####maybe better use followers
                else: self.pop = pop
                self.urm = urm
                self.binary = binary
                ...
                if self.similarity=='p3alpha': self.m_ui = pre.bm25_col(urm.copy()).tocsr()
                else: self.m_ui = urm.copy().tocsr()
                if self.similarity=='p3alpha': self.m_iu = pre.bm25_col(urm_aux.T.copy()).tocsr()
                else: self.m_iu = urm_aux.T.copy().tocsr()
                if mode == 'offline':
                        self.ev = Evaluator(self.dr)
        
        def model(self, alpha=1, beta=0, k=200, shrink=0, power=1, threshold=0, rp3_mode=0, target_items=None):
                ...
                if self.similarity=='tversky':
                        ...
                        self.s = ss.p3alpha_similarity(self.m_ui, self.m_iu,
                        k=k, shrink=shrink, alpha=alpha, threshold=threshold,
                        verbose=self.verbose, target_items=target_items)
                elif self.similarity=='rp3beta': 
                        self.s = p3r3.p3alpha_rp3beta_similarity(self.m_ui, self.m_iu, self.pop,
                        k=k, shrink=shrink, alpha=alpha, beta=beta, threshold=threshold,
        ...
        def __str__(self):
                name = ('CF_UB_BM25: alpha=%.3f, beta=%.3f, k=%d, shrink=%d, power=%.3f, threshold=%.5f, binary=%s, rp3mode=%d' 
                        % (self.alpha, self.beta, self.k , self.shrink, self.power, self.threshold, str(self.binary), self.rp3_mode))
                return name
