...
class SparseMatrixMultiplier {
 public:
    explicit SparseMatrixMultiplier(Index item_count, Value * denColArray_, Value shrink_ , Value threshold_)
        : sums(item_count, 0) ,nonzeros(item_count, -1), denColArray(denColArray_), shrink(shrink_), threshold(threshold_), denRow(0), head(-2), length(0) {
    }
    ...
    void foreach(Function & f) {  // NOLINT(*)
        ...
        for (int i = 0; i < length; ++i) {
            Index index = head;
            Value val = sums[index] / ((denRow*denColArray[index])+shrink);
            if (val >= threshold)
                f(index, val);
