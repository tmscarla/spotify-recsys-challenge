...
cdef extern from "cosine.h" namespace "similarity" nogil:
    cdef cppclass TopK[Index, Value]:
        TopK(size_t K)
        vector[pair[Value, Index]] results

    cdef cppclass SparseMatrixMultiplier[Index, Value]:
        SparseMatrixMultiplier(Index item_count, Value * denColArray_, Value shrink_, Value threshold)
...
def cosine_similarity(items, float alpha = 0.5, unsigned int k=100, float shrink=0, float threshold=0, binary=False ,int num_threads=0, target_items=[]):
    ...
    # save the data (not binary use set theorhy)
    if binary:
    ...
    # build the normalization terms
    cdef float[:] det_item = np.array(np.power(items.power(2).sum(axis = 1).A1, alpha), dtype=np.float32)
    cdef float[:] det_user = np.array(np.power(users.power(2).sum(axis = 0).A1, (1-alpha)), dtype=np.float32)
