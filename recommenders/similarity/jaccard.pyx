...
cdef extern from "jaccard.h" namespace "similarity" nogil:
...
def tanimoto_similarity(items, unsigned int k=100, float shrink=0, float threshold=0):
    return jaccard_similarity(items, k, shrink, binary=False)
        

@cython.boundscheck(False)
@cython.wraparound(False)
def jaccard_similarity(items, unsigned int k=100, float shrink=0, float threshold=0, binary=True, int num_threads=0):
    ...
    # save the data (jaccard use set theorhy)
    if binary:
