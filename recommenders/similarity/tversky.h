...
class SparseMatrixMultiplier {
 public:
    explicit SparseMatrixMultiplier(Index item_count, Value * denColArray, Value alpha, Value beta, Value shrink, Value threshold)
        : sums(item_count, 0) ,nonzeros(item_count, -1), denColArray(denColArray), alpha(alpha), beta(beta), shrink(shrink), threshold(threshold), denRow(0), head(-2), length(0) {
    }
    ...
    void foreach(Function & f) {  // NOLINT(*)
        ...
        for (int i = 0; i < length; ++i) {
            Index index = head;
            Value val= sums[index] / (alpha*(denRow-sums[index])+ beta*(denColArray[index]-sums[index])+ sums[index] +shrink);
            if (val >= threshold)
                f(index, val);
 ...
 protected:
    ...
    Value* denColArray;
    Value alpha, beta, shrink, threshold;
    Value denRow;
    Index head, length;
};
}  // namespace similarity
