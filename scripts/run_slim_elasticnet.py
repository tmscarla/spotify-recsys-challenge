...
if __name__ == '__main__':
    ...
    try:
        ######################SHRINKED
        dr = Datareader(mode="offline", train_format="200k", only_load=True)
        ev = Evaluator(dr)
        ...
        ######### FULL
        # dr = Datareader(mode="offline", train_format="400k", only_load=True, verbose=False)
        # ev = Evaluator(dr)
        # pids = dr.get_test_pids()
        # urm = dr.get_urm()
        #
        ...
        slim = SLIM_ElasticNet(urm)
        slim.fit(l1_penalty=l1, l2_penalty=l2, positive_only=True, topK=100)
