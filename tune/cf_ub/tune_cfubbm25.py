...
from recommenders.cf_ub_bm25 import CF_UB_BM25

dr = Datareader(mode='offline', only_load=True, verbose=False)
urm = dr.get_urm()
pids = dr.get_test_pids()
rec = CF_UB_BM25(urm, binary=True, datareader=dr, mode='offline', verbose=True, verbose_evaluation= False)
rec.tune_alpha_beta(k=400, verbose_tune=True, save_mean=True, overwrite=True, range_alpha=np.arange(0.4,1.1,0.1), range_beta= np.arange(-0.1,0.2,0.1))
print('DONE')